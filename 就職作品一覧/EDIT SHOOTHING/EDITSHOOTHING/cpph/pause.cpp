...
//マクロ定義
#define NUM_PAUSE (5)				//ポーズメニューの最大数
...
void InitPause(void)
{
	...
	for (nCntPAUSE = 0; nCntPAUSE < NUM_PAUSE; nCntPAUSE++)
	{
		...
		case PAUSE_MAX:
			...
			break;
		case PAUSE_RULE:
			pVtx[0].pos = D3DXVECTOR3(900.0f, 100.0f, 0.0f);
			pVtx[1].pos = D3DXVECTOR3(1200.0f,100.0f, 0.0f);
			pVtx[2].pos = D3DXVECTOR3(900.0f, 450.0f, 0.0f);
			pVtx[3].pos = D3DXVECTOR3(1200.0f,450.0f, 0.0f);
			//頂点カラー
			pVtx[0].col = g_ColSelect;
			pVtx[1].col = g_ColSelect;
			pVtx[2].col = g_ColSelect;
...
void DrawPause(void)
{
	...
	pDevice->DrawPrimitive(D3DPT_TRIANGLESTRIP, 4 * 4, 2);
